...
namespace icdtFramework.Plugins.ckeditor
{
    public partial class LinkBrowser : System.Web.UI.Page
    {
        ...
        /// 傳遞出訊息
        /// </summary>
        /// <param name="_strMEssage">訊息內容</param>
        /// <param name="_strJavaScript">要再產生的 Script 語法</param>
        void _OutMessage(string _strMEssage, string _strJavaScript)
        {
            ScriptManager.RegisterClientScriptBlock(this.Page, typeof(string), Guid.NewGuid().ToString(),
            "alert('" + _strMEssage + "');" + _strJavaScript, true);
        }
        ...
        protected void SelectImage(object sender, EventArgs e)
        {
            ...
            if (pos == -1)
                return;
            RenameImageButton.OnClientClick = "var name = prompt('請輸入檔案名稱(建議以英文字為主)：','" + ImageList.SelectedValue.Substring(0, pos) + "'); if (name == null || name == '') return false; document.getElementById('" + NewImageName.ClientID + "').value = name + '" + ImageList.SelectedValue.Substring(pos) + "';";
        ...
        protected void Upload(object sender, EventArgs e)
        {
            ...
            if (UploadedImageFile.PostedFile.ContentLength != 0)
            {
                filename = UniqueFilename(UploadedImageFile.FileName);
                    ...
                    Response.Redirect("LinkBrowser.aspx");
            }
            else
            {
                _OutMessage("請選擇要上傳的檔案!!", "");
            }
        }
        ...
        {
            get { return ResolveUrl(WebConstants.CkeditorFilesPath); }
        }
