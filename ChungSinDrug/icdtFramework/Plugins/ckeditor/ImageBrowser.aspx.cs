...
namespace icdtFramework.Plugins.ckeditor
{
    public partial class ImageBrowser : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                NewDirectoryButton.OnClientClick = "var name = prompt('請輸入目錄名稱(建議以英文字為主)：'); if (name == null || name == '') return false; document.getElementById('" + NewDirectoryName.ClientID + "').value = name;";
            }
        ...
        protected void BindDirectoryList()
        {
            ...
            DirectoryList.Items.Insert(0, "主目錄");
            ...
            try
            {
                ...
                if (Request["DirectName"] != null)
                {
                    DirectoryList.SelectedItem.Selected = false;
                    DirectoryList.Items.FindByValue(Request["DirectName"].ToString()).Selected = true;

                    ChangeDirectory(sender, e);
                }
        ...
        protected void DeleteFolder(object sender, EventArgs e)
        {
            ...
            Response.Redirect("ImageBrowser.aspx");
        }
        ...
        protected void BindImageList()
        {
            ...
            if (!IsPostBack)
            {
                if (Request["FilesName"] != null)
                {
                    ImageList.SelectedItem.Selected = false;
                    ImageList.Items.FindByValue(Request["FilesName"].ToString()).Selected = true;
                }
            }
        }
        ...
        protected void SelectImage(object sender, EventArgs e)
        {
            ...
            if (pos == -1)
                return;
            RenameImageButton.OnClientClick = "var name = prompt('請輸入檔案名稱(建議以英文字為主)：','" + ImageList.SelectedItem.Text.Substring(0, pos) + "'); if (name == null || name == '') return false; document.getElementById('" + NewImageName.ClientID + "').value = name + '" + ImageList.SelectedItem.Text.Substring(pos) + "';";
        ...
        protected void Upload(object sender, EventArgs e)
        {
            if (IsImage(UploadedImageFile.FileName))
            {
                ...
                Response.Redirect("ImageBrowser.aspx?DirectName=" + DirectoryList.SelectedItem.Value + "&FilesName=" + filename);
            }
        }
        ...
        {
            get { return ResolveUrl(WebConstants.CkeditorImagePath); }
        }
